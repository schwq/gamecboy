...
void bit_operation_swap_u16(u16 reg) {
    write_uint8_data(reg, (read_uint8_data(reg) >> 4) | ((read_uint8_data(reg) & 0xF) << 4));
    ...
    u8 prefix = read_uint8_data(cpu.reg.pc++);
     switch(prefix) {
        ...
        case 0x07:  // Rotate contents of A register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.a, true); // Carry = true (Rotate left with Carry RLC A)
            cycle_cpu(8);
            break;
        case 0x00:  // Rotate contents of B register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.b, true); // Carry = true (Rotate left with Carry RLC B)
            cycle_cpu(8);
            break;
        case 0x01:  // Rotate contents of C register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.c, true); // Carry = true (Rotate left with Carry RLC C)
            cycle_cpu(8);
            break;
        case 0x02:  // Rotate contents of D register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.d, true); // Carry = true (Rotate left with Carry RLC D)
            cycle_cpu(8);
            break;
        case 0x03:  // Rotate contents of E register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.e, true); // Carry = true (Rotate left with Carry RLC E)
            cycle_cpu(8);
            break;
        case 0x04:  // Rotate contents of H register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.h, true); // Carry = true (Rotate left with Carry RLC H)
            cycle_cpu(8);
            break;
        case 0x05:  // Rotate contents of L register left and store Bit 7 in CF, flags updated
            bit_operation_RL(&cpu.reg.l, true); // Carry = true (Rotate left with Carry RLC L)
            cycle_cpu(8);
            break;
        case 0x06:  // Rotate contents of byte at (HL) left and store Bit 7 in CF, flags updated
            bit_operation_RL_u16(cpu_read_reg(reg_hl), true); // Carry = true (Rotate left with Carry RLC L)
            cycle_cpu(16);
            break;
        ...
        case 0x18:  // Rotate contents of B register right and store Bit 0 in CF, flags updated
            bit_operation_RR(&cpu.reg.b, false); // Carry = false (Rotate right with Carry RR B)
            cycle_cpu(8);
            break;
        case 0x19:  // Rotate contents of C register right and store Bit 0 in CF, flags updated
            bit_operation_RR(&cpu.reg.c, false); // Carry = false (Rotate right with Carry RR C)
            cycle_cpu(8);
            break;
        case 0x1a:  // Rotate contents of D register right and store Bit 0 in CF, flags updated
            bit_operation_RR(&cpu.reg.d, false); // Carry = false (Rotate right with Carry RR D)
            cycle_cpu(8);
            break;
        case 0x1b:  // Rotate contents of E register right and store Bit 0 in CF, flags updated
            bit_operation_RR(&cpu.reg.e, false); // Carry = false (Rotate right with Carry RR E)
            cycle_cpu(8);
            break;
        case 0x1c:  // Rotate contents of H register right and store Bit 0 in CF, flags updated
            bit_operation_RR(&cpu.reg.h, false); // Carry = false (Rotate right with Carry RR H)
            cycle_cpu(8);
            break;
        case 0x1d:  // Rotate contents of L register right and store Bit 0 in CF, flags updated
            bit_operation_RR(&cpu.reg.l, false); // Carry = false (Rotate right with Carry RR L)
            cycle_cpu(8);
            break;
        case 0x1e:  // Rotate contents of byte at (HL) right and store Bit 0 in CF, flags updated
            bit_operation_RR_u16(cpu_read_reg(reg_hl), false); // Carry = false (Rotate right with Carry RR (HL))
            cycle_cpu(16);
            break;
       ...
       case 0xf7:  // Set Bit 6 in register A, flags not affected
        ...
        case 0xbe:  // Reset Bit 7 in byte at (HL), flags not affected
            ...
            break;
        default:
             _ERROR("in_cb_prefix_proc_error_invalid_immediate_u8, cannot load instruction PC: " PRINTF_ERROR_PC_REG)
            _CRITICAL
    }
}
